...
with left:
    ...
    permis_conduire = st.selectbox("Permis de conduire", [1, 0], format_func=lambda x: "Oui" if x == 1 else "Non")
    # Mapping binaire basé sur la position réelle dans les catégories du pkl
    ancien_map = {cat: ("Oui (déjà assuré)" if i == len(ancien_assure_cats)-1 else "Non (jamais assuré)") for i, cat in enumerate(ancien_assure_cats)}
    ancien_assure = st.selectbox("Déjà assuré ailleurs ?", ancien_assure_cats, format_func=lambda x: ancien_map[x])
...
with right:
    ...
    veh_map = {cat: ("Oui (endommagé)" if i == len(veh_endommage_cats)-1 else "Non (intact)") for i, cat in enumerate(veh_endommage_cats)}
    vehicule_endommage = st.selectbox("Véhicule endommagé ?", veh_endommage_cats, format_func=lambda x: veh_map[x])
